...
from emp inner join dept
--on emp.deptno=dept.deptno
USING(deptno) -- 공통컬럼은 하나만 가지게 해준다. 공통컬럼을 구별할 필요 X
;

-- NATURAL JOIN : 동일 컬럼 조인의 조건 생략, 같은 이름의 컬럼은 하나씩
select *
from emp NATURAL JOIN dept
;

-- Nom Equi JOIN : 동등 비교가 아닌 비교연산 가능
--급여 등급을 5개로 나누어 놓은 salgrade에서 정보를 얻어 와서
-- 각 사원의 급여 등급을 지정해보도록 합시다.
-- 이를 위해서 사원(emp) 테이블과 급여 등급(salgrade) 테이블을 조인하도록 합시다.
-- 다음은 사원의 급여가 몇 등급인지 살펴보자
...
-- 스칼라 부속질의 : select 절 프리젠테이션 영역에 쓸수 있는 
--                  부속질의, 단일행, 단일열, 단일값의 결과만 나와야한다.
...
where saleprice >
    (select avg(saleprice) from orders where custid=4)
  -- custid=1이면 1번 고객의 평균과 비교해서 큰 주문 금액만 출력
;
